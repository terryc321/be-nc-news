...
describe("convertTimestampToDate", () => {
  ...
  xtest("converts a created_at property to a date", () => {
    const timestamp = 1557572706232;
    const input = { created_at: timestamp };
    const result = convertTimestampToDate(input);
  ...
  xtest("ignores includes any other key-value-pairs in returned object", () => {
    const input = { created_at: 0, key1: true, key2: 1 };
    const result = convertTimestampToDate(input);
    expect(result.key1).toBe(true);
    expect(result.key2).toBe(1);
  });
  xtest("returns unchanged object if no created_at property", () => {
...
describe("createRef", () => {
  xtest("returns an empty object, when passed an empty array", () => {
    const input = [];
    const actual = createRef(input);
    const expected = {};
    expect(actual).toEqual(expected);
  });
  xtest("returns a reference object when passed an array with a single items", () => {
  ...
  xtest("returns a reference object when passed an array with many items", () => {
    const input = [
...
describe("formatComments", () => {
  xtest("returns an empty array, if passed an empty array", () => {
    const comments = [];
    expect(formatComments(comments, {})).toEqual([]);
    expect(formatComments(comments, {})).not.toBe(comments);
  });
  xtest("converts created_by key to author", () => {
  ...
  xtest("replaces belongs_to value with appropriate id when passed a reference object", () => {
  ...
  xtest("converts created_at timestamp to a date", () => {
